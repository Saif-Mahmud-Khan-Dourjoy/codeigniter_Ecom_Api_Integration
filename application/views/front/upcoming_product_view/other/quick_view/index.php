...
<?php  
	...
	$thumbs = $this->crud_model->file_view('upcoming_product',$row['upcoming_product_id'],'','','thumb','src','multi','all');
	$mains = $this->crud_model->file_view('upcoming_product',$row['upcoming_product_id'],'','','no','src','multi','all'); 
?>
<section class="page-section">
    <div class="row product-single" style="margin-top: 0px;">
        ...
        <div class="col-md-7">
            ...
            <?php
            if ($this->db->get_where('upcoming_product', array('upcoming_product_id' => $row['upcoming_product_id']))->row()->is_bundle == 'no') {
            ?>
            ...
            ?>
            <?php
            if ($this->db->get_where('upcoming_product', array('upcoming_product_id' => $row['upcoming_product_id']))->row()->is_bundle == 'yes') {
            ?>
            <div style="padding: 5px">
                ...
                <?php
                    ...
                    foreach ($products as $product) { ?>
                        ...
                        <a style="text-decoration:underline; font-size: 12px; padding-left: 15px;" href="<?php echo $this->crud_model->product_link1($product['upcoming_product_id']); ?>">
                            <?php echo $this->db->get_where('upcoming_product', array('upcoming_product_id' => $product['upcoming_product_id']))->row()->title . '<br>';?>
                        </a>
                <?php
                    }
                ?>
            ...
            <div class="added_by">
                ...
                <p>
                    <?php echo $this->crud_model->product_by($row['upcoming_product_id'],'with_link');?>
                </p>
            </div>
            <?php } ?>
            <div class="product-rating clearfix">
                <?php $rating = $this->crud_model->rating($row['upcoming_product_id']); ?>
                <div  class="rateit" data-rateit-value="<?= $rating ?>" data-rateit-ispreset="true" data-rateit-readonly="true"></div>
                <div style="display: none" class="rating ratings_show" data-original-title="<?php echo $rating = $this->crud_model->rating($row['upcoming_product_id']); ?>"
                ...
                <div class="rating inp_rev list-inline" style="display:none;" data-pid='<?php echo $row['upcoming_product_id']; ?>'>
                            ...
                            foreach ($user_products as $user_prod) {
                                foreach($user_prod as $prods){
                                    ...
                                    foreach($prods as $prod){
                                        //echo $prod['id'];
                                        if($prod['id'] == $row['upcoming_product_id']){
                                            //echo $prod['id'];
                                            $is_review = 'yes';
                                        }
                                    }
            ...
            <h4>
                <a style="text-decoration:underline;" href="<?php echo $this->crud_model->product_link1($row['upcoming_product_id']); ?>">
