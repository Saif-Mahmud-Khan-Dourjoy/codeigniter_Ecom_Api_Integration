...
{
    /*  
     *  Developed by: DevsZone
     *  Date    : 14 July, 2015
     *  DevsZone eCommerce CMS
     *  
     */
    ...
    function product($para1 = '', $para2 = '', $para3 = '')
    {
        ...
        /* Upcoming Product add, edit, view, delete, stock increase, decrease, discount */
                function upcoming_product($para1 = '', $para2 = '', $para3 = '')
        {
            if (!$this->crud_model->admin_permission('product')) {
                redirect(base_url() . 'admin');
            }
            ...
            if ($para1 == 'do_add') {
                ...
                $this->db->insert('upcoming_product', $data);
                $id = $this->db->insert_id();
                $this->benchmark->mark_time();
                $this->crud_model->file_up("images", "upcoming_product", $id, 'multi');
                $this->crud_model->set_category_data(0);
                recache();
            } else if ($para1 == "update") {
                $options = array();
                ...
                $num                        = $this->crud_model->get_type_name_by_id('upcoming_product', $para2, 'num_of_imgs');
                $data['title']              = $this->input->post('title');
                $data['description']        = $this->input->post('description');   
                ...
                $data['num_of_imgs']        = $num + $num_of_imgs;
                $data['front_image']        = 0;
                $this->crud_model->file_up("images", "upcoming_product", $para2, 'multi');
                
                $this->db->where('upcoming_product_id', $para2);
                $this->db->update('upcoming_product', $data);
                $this->crud_model->set_category_data(0);
                recache();
            } else if ($para1 == 'edit') {
                $page_data['product_data'] = $this->db->get_where('upcoming_product', array(
                    'upcoming_product_id' => $para2
                ))->result_array();
                $this->load->view('back/admin/upcoming_product_edit', $page_data);
            } else if ($para1 == 'view') {
                $page_data['product_data'] = $this->db->get_where('product', array(
                    'product_id' => $para2
                ))->result_array();
                ...
                $this->crud_model->file_dlt('upcoming_product', $para2, '.jpg', 'multi');
                $this->db->where('upcoming_product_id', $para2);
                $this->db->delete('upcoming_product');
                $this->crud_model->set_category_data(0);
                recache();
            } elseif ($para1 == 'list') {
                $this->db->order_by('upcoming_product_id', 'desc');
                $this->db->where('download=',NULL);
                $page_data['all_product'] = $this->db->get('upcoming_product')->result_array();
                $this->load->view('back/admin/upcoming_product_list', $page_data);
            } elseif ($para1 == 'list_data') {
                $limit      = $this->input->get('limit');
                $search     = $this->input->get('search');
                $order      = $this->input->get('order');
                ...
                $total      = $this->db->get('upcoming_product')->num_rows();
                $this->db->limit($limit);
                if($sort == ''){
                    $sort = 'upcoming_product_id';
                    $order = 'DESC';
                }
                $this->db->order_by($sort,$order);
                if($search){
                ...
                $products   = $this->db->get('upcoming_product', $limit, $offset)->result_array();
                $data       = array();
                foreach ($products as $row) {
                    ...
                    $res['image']  = '<img class="img-sm" style="height:auto !important; border:1px solid #ddd;padding:2px; border-radius:2px !important;" src="'.$this->crud_model->file_view('upcoming_product',$row['upcoming_product_id'],'','','thumb','src','multi','one').'"  />';
                    $res['title']  = $row['title'];
                    ...
                    $res['options'] = "
                                <a class=\"btn btn-success btn-xs btn-labeled fa fa-wrench\" data-toggle=\"tooltip\" 
                                    onclick=\"ajax_set_full('edit','".translate('edit_product')."','".translate('successfully_edited!')."','product_edit','".$row['upcoming_product_id']."');proceed('to_list');\" data-original-title=\"Edit\" data-container=\"body\">
                                        ".translate('edit')."
                                </a>
                                
                                <a onclick=\"delete_confirm('".$row['upcoming_product_id']."','".translate('really_want_to_delete_this?')."')\" 
                                    class=\"btn btn-danger btn-xs btn-labeled fa fa-trash\" data-toggle=\"tooltip\" data-original-title=\"Delete\" data-container=\"body\">
                                        ".translate('delete')."
                                </a>";
                    $data[] = $res;
                ...
                $a = explode('_', $para2);
                $this->crud_model->file_dlt('upcoming_product', $a[0], '.jpg', 'multi', $a[1]);
                recache();
            } elseif ($para1 == 'add') {
                $this->load->view('back/admin/upcoming_product_add');
            } else {
                $page_data['page_name']   = "upcoming_product";
                $page_data['all_product'] = $this->db->get('upcoming_product')->result_array();
                $this->load->view('back/index', $page_data);
            }
        }
    ...
    function banner($para1 = '', $para2 = '', $para3 = '')
    {
        ...
        /*Frontend Upcoming Product Management */
        function upcoming($para1 = '', $para2 = '', $para3 = '')
        {
            if (!$this->crud_model->admin_permission('banner')) {
                redirect(base_url() . 'admin');
            }
            if ($para1 == "set") {
                ...
                if($_FILES['img']['name'] !== ''){
                    ...
                    $ext = pathinfo($path, PATHINFO_EXTENSION);
                    $data['image_ext']       = '.'.$ext;
                    $this->crud_model->file_up("img", "upcoming", $para2, '','','.'.$ext);
                }
                $this->db->where('banner_id', $para2);
                $this->db->update('upcoming_product', $data);
                $this->crud_model->file_up("img", "banner", $para2);
